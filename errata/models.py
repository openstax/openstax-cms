...
INTERNAL_EDITORIAL_REVIEW = 'Internal Editorial Review'
REVIEWED = 'Reviewed'
COMPLETED = 'Completed'
ERRATA_STATUS = (
    (NEW, 'New'),
...
class Errata(models.Model):
    ...
    def clean(self):
        ...
        if (self.status == 'Editorial Review' or self.status == 'K-12 Editorial Review' or self.status == 'Internal Editorial Review' or self.status == 'Reviewed' or self.status == 'Completed') and not self.is_assessment_errata:
