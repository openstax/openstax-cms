...
def news_article_collection_search(collection, content_types=None, subjects=None):
    ...
    for na in news_articles:
        if collection is not None and len(na.blog_collections) > 0 and collection in na.blog_collections[0]['name']:
        ...
        if len(content_types) > 0 and len(subjects) > 0:
            for article in collection_articles:
                ...
                for item in subjects:
                    if len(blog_subjects) > 0 and item in blog_subjects[0]['name'] and not added:
                        articles_to_return.append(article)
            ...
            for article in collection_articles:
                blog_subjects = article.blog_subjects
                for item in subjects:
                    if len(blog_subjects) > 0 and item in blog_subjects[0]['name']:
                        articles_to_return.append(article)
        else:
...
def news_article_subject_search(subject):
    ...
    for article in news_articles:
        blog_subjects = article.blog_subjects
        if len(blog_subjects) > 0 and subject in blog_subjects[0]['name']:
            articles_to_return.append(article)
    return articles_to_return
